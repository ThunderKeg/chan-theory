...
站点同时是可安装的 PWA：移动端可添加到桌面，首次只缓存阅读器骨架；打开某章时，才按章节缓存该章 JSON 和该章图片。阅读进度保存在本机，重新打开会恢复章节和具体阅读位置。每课提供正文朗读音频，音频文件作为普通静态资源托管，避免写入离线 shell 缓存。阅读页面默认申请屏幕常亮，也可通过顶部“常亮”按钮关闭并记住选择。点击原书图片可进入支持双指、滚轮缩放和拖动的查看器。页面会在启动、回到前台、网络恢复和阅读期间定时检查新版；检测到新版时显示“发现新版本”，由读者点击后刷新，避免阅读中断。
